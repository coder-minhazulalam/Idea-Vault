      ...
      <form onSubmit={handleComment}>
        ...
        <div className="mt-4 flex justify-end">
          <button
            type="submit"
            disabled={isPosting}
            className="rounded-xl bg-amber-600 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-amber-500 disabled:cursor-not-allowed disabled:opacity-60 cursor-pointer"
          >
              ...
              >
                <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900">
                  <div className="flex items-start gap-3">
                    <img
                      src={item.userImage || user?.image || "/default-avatar.png"}
                      alt={item.userName || user?.name || "User"}
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          <p className="text-sm font-semibold text-zinc-900 dark:text-white">
                            {item.userName || user?.name || "Anonymous"}
                          </p>
                ...
                <div className="mt-3 flex justify-end gap-2">
                  <EditComments comment={item} onSuccess={fetchComments} />
                  <DeleteComments commentId={item._id} onSuccess={fetchComments} />
                </div>
              </div>
            ))}
          </div>
